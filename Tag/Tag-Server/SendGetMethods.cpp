...
bool Server::SendVector(int ID ,std::string & _v)
{
	if (!SendPacketType(ID, P_PlayerData)) //Send packet type: Chat Message, If sending packet type fails...
	...
	if (!sendall(ID, (char*)_v.c_str(), bufferlength)) //Try to send string buffer... If buffer fails to send,
		return false; //Return false: Failed to send string buffer
	return true; //Return true: string successfully sent
}

bool Server::GetVector(int ID, std::string & _v)
{
...
{
	std::string _string = std::to_string(ID);
	if (!SendPacketType(ID, P_PlayerNo)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	int bufferlength = _string.size(); //Find string buffer length
	if (!SendInt(ID, bufferlength)) //Send length of string buffer, If sending buffer length fails...
		return false; //Return false: Failed to send string buffer length
